...
const playlistId = "RDsy8AEmqj0-M";

async function addMixMusics() {
  console.log("A obter informações da mix no YouTube...");
  ...
  try {
    ...
    if (!data.items || data.items.length === 0) {
      console.log("Nenhum vídeo encontrado na mix.");
      process.exit(1);
    }
    ...
    for (const item of data.items) {
      ...
      await addDoc(collection(db, 'channels'), {
        ...
        description: title,
        youtubeId: videoId,
        // Gera um mix automático para este vídeo para tocar continuamente
    ...
    console.log(`Todos os ${data.items.length} vídeos foram adicionados com sucesso à categoria Músicas!`);
    process.exit(0);
